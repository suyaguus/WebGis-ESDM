...
import type { AnalyticsTrendFilter, TrendDataPoint, CompanyAnalytics } from '@/types/api';

const USE_MOCK = import.meta.env.VITE_USE_MOCK === 'true';
const mockDelay = () => new Promise(r => setTimeout(r, 350));

export const analyticsService = {
  getTrend: async (filter?: AnalyticsTrendFilter): Promise<TrendDataPoint[]> => {
    if (USE_MOCK) {
      await mockDelay();
      const count = filter?.period === '6m' ? 6 : filter?.period === '2y' ? 24 : 12;
      return ANALYTICS_MONTHLY.slice(-count).map(d => ({
        label: d.month, sw: d.sw, gnss: d.gnss, threshold: d.threshold,
      }));
    }
    const { data } = await api.get<TrendDataPoint[]>('/analytics/trend', { params: filter });
    return data;
  },
  ...
  getCompanySummary: async (): Promise<CompanyAnalytics[]> => {
    ...
    const { data } = await api.get<CompanyAnalytics[]>('/analytics/company-summary');
    return data;
  },
};
