...
function WellStatusBadge({ status }: { status: WellStatus }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 text-[9px] font-mono font-semibold px-2 py-0.5 rounded-full border", cfg.cls)}>
      {cfg.icon} {cfg.label}
    </span>
  );
}
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 space-y-2 max-h-[60vh] overflow-y-auto">
          {sensor.wellStatus === "rejected" && sensor.supervisorNote && (
            <div className="flex gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-red-600 font-mono leading-relaxed">
                  {sensor.supervisorNote}
                </p>
              </div>
            </div>
          )}
...
function CreateSensorModal({
  ...
  const formatDateTimeForBackend = (value: string | undefined): string | undefined => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-96 overflow-y-auto">
          ...
          <AdminSensorFieldWrapper label="Unit Bisnis" error={errors.businessId}>
            <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <AdminSensorFieldWrapper label="Latitude" error={errors.latitude}>
              <input
                ...
                value={form.latitude ?? ""}
                onChange={(e) =>
                  setField("latitude", e.target.value ? parseFloat(e.target.value) : undefined)
                }
            ...
            <AdminSensorFieldWrapper label="Longitude" error={errors.longitude}>
              <input
                ...
                onChange={(e) =>
                  setField("longitude", e.target.value ? parseFloat(e.target.value) : undefined)
                }
          ...
          <div className="grid grid-cols-2 gap-3">
            <AdminSensorFieldWrapper label="Kedalaman (m)">
              <input
                ...
                onChange={(e) =>
                  setField("depthMeter", e.target.value ? parseFloat(e.target.value) : undefined)
                }
                placeholder="50"
                className={getAdminSensorInputCls()}
              />
            ...
            <AdminSensorFieldWrapper label="Diameter (inch)">
              <input
                ...
                onChange={(e) =>
                  setField("diameterInch", e.target.value ? parseFloat(e.target.value) : undefined)
                }
                placeholder="6"
                className={getAdminSensorInputCls()}
              />
          ...
          <div className="grid grid-cols-2 gap-3">
            <AdminSensorFieldWrapper label="Kapasitas Pompa">
              <input
                ...
                onChange={(e) =>
                  setField("pumpCapacity", e.target.value ? parseFloat(e.target.value) : undefined)
                }
                placeholder="100"
                className={getAdminSensorInputCls()}
              />
            ...
            <AdminSensorFieldWrapper label="Muka Air Tanah (cm)">
              <input
                ...
                onChange={(e) =>
                  setField("staticWaterLevelCm", e.target.value ? parseFloat(e.target.value) : undefined)
                }
                placeholder="250.50"
                className={getAdminSensorInputCls()}
              />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <AdminSensorFieldWrapper label="Tanggal Pengukuran (Opsional)">
              <input
                ...
                onChange={(e) =>
                  setField("lastWaterLevelMeasurement", e.target.value || undefined)
                }
                className={getAdminSensorInputCls()}
              />
            </AdminSensorFieldWrapper>
...
function SensorEditModal({ sensor, onClose, businesses }: SensorEditModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    staticWaterLevelCm: (sensor.staticWaterLevel ? sensor.staticWaterLevel * 100 : "").toString(),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      diameterInch: form.diameterInch ? parseFloat(form.diameterInch) : undefined,
      pumpCapacity: form.pumpCapacity ? parseFloat(form.pumpCapacity) : undefined,
      staticWaterLevelCm: form.staticWaterLevelCm ? parseFloat(form.staticWaterLevelCm) : undefined,
      waterLevelTrend: form.waterLevelTrend || "stable",
      lastWaterLevelMeasurement: formatDateTimeForBackend(form.lastWaterLevelMeasurement),
    };
    updateSensor.mutate({ id: sensor.id, payload }, { onSuccess: onClose });
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <p className="text-[15px] font-bold text-slate-800">Edit Data Sumur</p>
            <p className="text-[10px] text-slate-400 font-mono mt-0.5">Perbarui informasi sumur</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition-colors"
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-96 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <AdminSensorFieldWrapper label="Tanggal Pengukuran (Opsional)">
              <input
                type="datetime-local"
                value={form.lastWaterLevelMeasurement ? form.lastWaterLevelMeasurement.toString().slice(0, 16) : ""}
                onChange={(e) => setField("lastWaterLevelMeasurement", e.target.value || undefined)}
                className={getAdminSensorInputCls()}
              />
            </AdminSensorFieldWrapper>
          </div>
...
/* ── Main Page ── */
export default function PengajuanSumurPage() {
  const [search, setSearch] = useState("");
  const [wellStatusF, setWellStatusF] = useState<WellStatus | "all">("all");
  ...
  const userBusinesses = allBusinesses.filter((b) => b.companyId === userCompanyId);

  const createSensor = useCreateSensor();

  const PENDING_SENSORS = COMPANY_SENSORS.filter((s) => s.wellStatus !== "approved");

  const data = useMemo(() => {
    let d = COMPANY_SENSORS.filter((s) => s.wellStatus !== "approved");
    if (wellStatusF !== "all") d = d.filter((s) => s.wellStatus === wellStatusF);
    ...
    return d;
  }, [search, wellStatusF, typeF, sortKey, sortAsc, COMPANY_SENSORS]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Ajukan dan daftarkan sumur baru perusahaan Anda
          </p>
        </div>
        <button
        ...
        >
          <Plus size={13} />
          <span className="hidden sm:block">Tambah Sumur</span>
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Total Pengajuan", value: PENDING_SENSORS.length, color: "#F59E0B", bg: "bg-amber-50 border-amber-200" },
              { label: "Pengajuan",        value: PENDING_SENSORS.filter((s) => s.wellStatus === "draft").length, color: "#D97706", bg: "bg-amber-50 border-amber-200" },
              { label: "Sedang Diproses",  value: PENDING_SENSORS.filter((s) => s.wellStatus === "pending_approval").length, color: "#3B82F6", bg: "bg-blue-50 border-blue-200" },
              { label: "Ditolak",          value: PENDING_SENSORS.filter((s) => s.wellStatus === "rejected").length, color: "#EF4444", bg: "bg-red-50 border-red-200" },
            ...
            <div className="flex flex-wrap items-center gap-2 px-3 md:px-4 py-3 border-b border-slate-100">
              ...
              <div className="flex gap-1">
                {(["all", "draft", "pending_approval", "approved", "rejected"] as const).map((s) => (
                  ...
                  >
                    {s === "all" ? "Semua" : WELL_STATUS_CONFIG[s as WellStatus]?.label ?? s}
                  </button>
                ))}
              </div>
              <div className="flex gap-1">
                ...
                <tbody className="divide-y divide-slate-50">
                  {data.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[11px] text-slate-600">{s.businessName ?? "-"}</td>
                      <td className="px-4 py-3 text-[11px] text-slate-600">{s.companyName}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          {s.waterLevelTrend === "rising" && <TrendingUp size={14} className="text-emerald-600" />}
                          {s.waterLevelTrend === "falling" && <TrendingDown size={14} className="text-amber-600" />}
                          {s.waterLevelTrend === "stable" && <Minus size={14} className="text-blue-600" />}
                      ...
                      <td className="px-4 py-3">
                        <WellStatusBadge status={s.wellStatus} />
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => setDetail(s)}
