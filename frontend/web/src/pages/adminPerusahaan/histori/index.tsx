...
export default function AdminHistoriPage() {
  ...
  const histori = COMPANY_MEASUREMENTS.filter(
    (m) => selectedSensor === "all" || m.sensorCode === selectedSensor,
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Riwayat data pengukuran lapangan
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {COMPANY_SENSORS.slice(0, 5).map((s) => (
          <button
            key={s.id}
            onClick={() =>
              setSelectedSensor(selectedSensor === s.code ? "all" : s.code)
            }
            className={cn(
              "bg-white rounded-xl border shadow-sm p-3 text-left transition-all hover:border-amber-300",
              selectedSensor === s.code
                ? "border-amber-400 ring-1 ring-amber-300"
                : "border-slate-100",
            )}
          >
            <p className="text-[11px] font-bold font-mono text-slate-800 mb-0.5">
              {s.code}
            </p>
            <p className="text-[10px] font-mono text-slate-600">
              Muka Air: {s.staticWaterLevel ? `${s.staticWaterLevel}m` : "-"}
            </p>
            <div className="mt-2">
              <MiniHistoryChart
                color={
                  s.status === "alert"
                    ? "#EF4444"
                    : s.staticWaterLevel && s.staticWaterLevel <= -32
                      ? "#F59E0B"
                      : "#22C55E"
                }
                data={COMPANY_TREND_DATA.map(
                  (d) => d.waterLevel + (Math.random() * 0.3 - 0.15),
                )}
              />
            </div>
          </button>
            ...
            <tbody className="divide-y divide-slate-50">
              {histori.map((m) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-[12px] font-bold font-mono text-slate-800">
                    {m.sensorCode}
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-1.5">
                      <div className="w-5 h-5 rounded-full bg-amber-100 flex items-center justify-center text-[8px] font-bold text-amber-700 flex-shrink-0">
                        {m.surveyorAvatar}
                      </div>
