...
export default function AdminDashboard() {
  ...
  const companyName = company?.name ?? user?.name ?? "Perusahaan";
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-[13px] font-semibold text-slate-800">
              Kuota Pengambilan Air Tanah 2026
            </p>
            <p className="text-[10px] text-slate-400 font-mono mt-0.5">
              {company
                ? `${(company.quotaUsed / 1000).toFixed(0)}k m³ terpakai dari ${(company.quota / 1000).toFixed(0)}k m³ total izin`
                : "—"}
            </p>
          </div>
        ...
        {pct >= 85 && (
          <div className="mt-3 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            ...
            <p className="text-[11px] text-amber-700">
              Kuota tersisa <strong>{100 - pct}%</strong> (
              {company
                ? ((company.quota - company.quotaUsed) / 1000).toFixed(0)
                : "—"}
              k m³). Hubungi Kepala Instansi jika memerlukan penambahan kuota.
            </p>
          </div>
        )}
      </div>
