...
export default function AdminMapSection() {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col min-w-0">
      ...
      {/* Map */}
      <div className="relative min-h-[250px] sm:min-h-[220px]" style={{ height: 'clamp(250px, 72vw, 360px)' }}>
        <SensorMap sensors={COMPANY_SENSORS} height="100%" onMarkerClick={setSelected} className="rounded-none sm:rounded-b-xl" />
        ...
        {selected && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-white border border-amber-200 rounded-xl shadow-lg px-4 py-3 z-[1000] min-w-[200px] hidden sm:block">
      ...
      <div className="sm:hidden border-t border-slate-100 bg-slate-50/60 px-3 py-3 space-y-3">
        <div>
          <p className="text-[9px] font-mono text-slate-400 uppercase tracking-wide mb-2">Legenda</p>
          <div className="grid grid-cols-2 gap-2">
            {[['#3B82F6','Air Tanah'],['#F59E0B','GNSS'],['#EF4444','Alert'],['#94A3B8','Maint']].map(([c, l]) => (
              <div key={l} className="flex items-center gap-2 bg-white border border-slate-100 rounded-lg px-2.5 py-1.5">
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: c }} />
        ...
        {selected && (
          <div className="bg-white border border-amber-200 rounded-xl shadow-sm px-3 py-3">
            <div className="flex items-center justify-between mb-1.5 gap-2">
              <span className="text-[12px] font-bold font-mono text-amber-700">{selected.code}</span>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-600 text-[11px]">Tutup</button>
            </div>
            <p className="text-[10px] text-slate-500 mb-2">{selected.location}</p>
            <div className="space-y-1">
              <div className="flex justify-between">
