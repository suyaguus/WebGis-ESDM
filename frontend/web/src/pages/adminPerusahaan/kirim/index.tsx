...
const INITIAL_DISPATCHES: DispatchItem[] = [
  {
    ...
    status: "in_progress",
    note: "Fokus pada lokasi dengan alert air tanah tinggi.",
  },
  {
    id: "d2",
...
export default function AdminKirimSurveyorPage() {
  const [dispatches, setDispatches] =
    useState<DispatchItem[]>(INITIAL_DISPATCHES);
  const [selectedId, setSelectedId] = useState<string>(
    INITIAL_DISPATCHES[0]?.id ?? "",
  );

  const selected = useMemo(
    () =>
      dispatches.find((item) => item.id === selectedId) ??
      dispatches[0] ??
      null,
  ...
  const summary = useMemo(
    () => ({
      ...
      inProgress: dispatches.filter((item) => item.status === "in_progress")
        .length,
  ...
  const createDraft = () => {
    ...
    const freeSensors = COMPANY_SENSORS.slice(0, 2).map(
      (sensor) => sensor.code,
    );
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Buat dan kirim paket tugas lapangan ke tim surveyor
          </p>
        </div>
        <button
          onClick={createDraft}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1 max-w-48">
                          {item.sensorCodes.map((code) => (
                            <span
                              key={code}
                              className="text-[9px] font-mono px-1.5 py-0.5 rounded border bg-slate-50 text-slate-600 border-slate-200"
                            >
                              {code}
                            </span>
                          ))}
        ...
        {selected ? (
          <Card padding={false}>
            <SectionHeader
              ...
              icon={<Users size={13} />}
              accent="#F59E0B"
              subtitle={selected.surveyorName}
            />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {selected.sensorCodes.map((code) => (
                    <span
                      key={code}
                      className="text-[9px] font-mono px-1.5 py-0.5 rounded border bg-amber-50 text-amber-700 border-amber-200"
                    >
                      {code}
                    </span>
                  ))}
