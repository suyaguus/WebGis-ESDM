...
function MeasurementRow({
  ...
  m: Measurement;
  onAction: (id: string, action: "verify" | "reject") => void;
}) {
    ...
    <>
      <tr
        className={cn(
          ...
          m.status === "pending" && "bg-amber-50/20",
        )}
      >
        ...
        <td className="px-4 py-3">
          <span
            className={cn(
              "text-[9px] font-mono font-semibold flex items-center gap-1",
              cfg.color,
            )}
          >
            {m.status === "pending" && (
                  ...
                  <div className="space-y-1">
                    ...
                    {m.verifiedAt && (
                      <div className="flex justify-between text-[10px]">
                        ...
                        <span className="font-mono">{m.verifiedAt}</span>
                      </div>
                    )}
                  </div>
                </div>
    ...
    <div className="p-3 md:p-5 space-y-3 md:space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Review pengukuran lapangan dari surveyor
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[12px] font-semibold text-amber-800">
                {pending} data pengukuran menunggu verifikasi
              </p>
              <p className="text-[11px] text-amber-600">
                Segera verifikasi untuk memastikan akurasi data sensor
              </p>
            </div>
          </div>
            ...
            <button
              onClick={() =>
                measurements
                  ...
                  .forEach((m) => handleAction(m.id, "verify"))
              }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Menunggu",
            value: measurements.filter((m) => m.status === "pending").length,
            color: "#F59E0B",
            bg: "bg-amber-50  border-amber-200",
          },
          {
            label: "Diverifikasi",
            value: measurements.filter((m) => m.status === "verified").length,
            color: "#22C55E",
            bg: "bg-emerald-50 border-emerald-200",
          },
          {
            label: "Ditolak",
            value: measurements.filter((m) => m.status === "rejected").length,
            color: "#EF4444",
            bg: "bg-red-50     border-red-200",
          },
          ...
          <div className="flex gap-1 ml-auto">
            {(["all", "pending", "verified", "rejected"] as const).map((s) => (
              ...
              >
                {s === "all"
                  ? "Semua"
                  : STATUS_CONFIG[s as Measurement["status"]]?.label}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full" style={{ minWidth: "760px" }}>
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((m) => (
                <MeasurementRow key={m.id} m={m} onAction={handleAction} />
              ))}
            </tbody>
          </table>
        </div>
