...
import { StatusPill, Card, SectionHeader, Badge, SeverityBadge } from '../../../components/ui';
...
function DetailModal({ sensor, onClose }: { sensor: Sensor; onClose: () => void }) {
  const co = MOCK_COMPANIES.find(c => c.id === sensor.companyId);
  return (
...
export default function SensorPage() {
  ...
  const sort = (k: SortKey) => { setSortKey(k); setSortAsc(sk => sk === k ? !sortAsc : true); };
  ...
  const summary = {
    ...
    alert: MOCK_SENSORS.filter(s => s.status === 'alert').length,
  ...
  return (
    <div className="p-3 sm:p-5 space-y-4">
        ...
        {[
          ...
          <div key={label} className={cn('rounded-xl border px-4 py-3 flex items-center gap-3', bg, border)}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: color + '20' }}>
              <span className="text-[14px] font-bold font-mono" style={{ color }}>{count}</span>
            </div>
                ...
                <tr key={s.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => setDetail(s)}
                      className="text-[10px] font-mono text-cyan-600 hover:text-cyan-800 font-medium whitespace-nowrap">Detail →</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {detail && <DetailModal sensor={detail} onClose={() => setDetail(null)} />}
    </div>
  );
}
