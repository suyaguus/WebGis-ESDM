...
export default function PetaPage() {
  ...
  const storeSelected    = useAppStore((s) => s.selectedSensor);
  const clearStoreSelect = useAppStore((s) => s.setSelectedSensor);

  // When navigated here from dashboard with a pre-selected sensor, pick it up
  useEffect(() => {
    if (storeSelected) {
      setSelected(storeSelected);
      clearStoreSelect(null);
    }
  }, []);
  ...
  return (
    <div className="flex h-full overflow-hidden" style={{ minHeight: 0 }}>
      {/* ── Sidebar ── */}
      {sidebarOpen && (
        <div className="w-72 flex-shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-hidden shadow-sm">
          {/* Header */}
