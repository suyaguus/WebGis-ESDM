...
export default function ConfigPage() {
  ...
  const update = <K extends keyof typeof config>(k: K, v: (typeof config)[K]) =>
    setConfig((c) => ({ ...c, [k]: v }));
  const handleSave = () => {
    setSaved(true);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Pengaturan global aplikasi SIGAT
          </p>
        </div>
        <button
          onClick={handleSave}
          className={cn(
            "px-4 py-2 text-[12px] font-semibold rounded-xl flex items-center gap-2 transition-all",
            saved
        ...
        <Card padding={false}>
          ...
          <div className="p-4 space-y-4">
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {["Street", "Satellite", "Terrain"].map((l) => (
                  <button
                    key={l}
                    onClick={() => update("mapDefault", l)}
                    className={cn(
                      "py-2 text-[10px] font-mono font-medium rounded-lg border transition-all",
                      config.mapDefault === l
            ...
            <div>
              ...
              <select
                value={config.tileServer}
                onChange={(e) => update("tileServer", e.target.value)}
            ...
            <div className="space-y-2.5">
              {[
                {
                  label: "Aktifkan Heatmap Layer",
                  key: "enableHeatmap" as const,
                },
                {
                  label: "Aktifkan Marker Clustering",
          ...
          <div className="p-4 space-y-4">
            <div>
              ...
              <input
                ...
                max={3650}
                onChange={(e) =>
                  update("dataRetention", parseInt(e.target.value))
                }
                className="w-full text-[12px] font-mono border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 text-slate-800 focus:outline-none focus:border-cyan-400"
              />
              <p className="text-[9px] text-slate-400 font-mono mt-1">
                Data lebih lama dari ini akan diarsipkan otomatis
              </p>
            </div>
            <div>
              <label className="text-[9px] font-mono text-slate-400 uppercase tracking-wider block mb-1.5">
