        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-mono mt-0.5 truncate max-w-[240px]">
                {reportName}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            ...
            <textarea
              ...
              className={cn(
                ...
                error
                  ...
                  : "border-slate-200 focus:ring-red-400 focus:border-red-400",
              )}
            />
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="flex-1 px-4 py-2 bg-red-600 text-white text-[12px] font-semibold rounded-xl hover:bg-red-700 transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-1.5"
            >
              {isPending ? (
                "Menolak..."
              ) : (
                <>
                  <X size={12} /> Tolak Laporan
                </>
              )}
            </button>
          </div>
...
export default function SuperAdminVerifikasiPage() {
  ...
  const [rejectTarget, setRejectTarget] = useState<VerificationReport | null>(
    null,
  );
  ...
  const detailRow = useMemo(
    () => reports.find((r) => r.id === detailId) ?? null,
    [reports, detailId],
  );
    ...
    data.sort((a, b) => {
      if (sortKey === "status") {
        return sortAsc
          ...
          : (STATUS_ORDER[b.status] ?? 99) - (STATUS_ORDER[a.status] ?? 99);
      }
      if (sortKey === "waterDepth")
        return sortAsc
          ? a.waterDepth - b.waterDepth
          : b.waterDepth - a.waterDepth;
    ...
    rejectReport.mutate(
      { id: tid, reason },
      {
        onSuccess: () => {
          ...
          if (detailId === tid) setDetailId(null);
        },
      },
    );
  };
  ...
  return (
    <div className="p-3 sm:p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Tinjau laporan surveyor sebelum masuk ke laporan resmi
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari sumur / perusahaan / surveyor..."
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[11px] text-slate-600 max-w-[160px] truncate">
                      {row.companyName}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[12px] font-semibold font-mono text-blue-700">
                        ...
                        {row.status === "PENDING" && (
                          <>
                            <button
                              ...
                              className="w-7 h-7 rounded-lg bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 flex items-center justify-center disabled:opacity-50"
                            >
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td
                      colSpan={8}
                      className="py-12 text-center text-[11px] text-slate-400 font-mono"
                    >
      ...
      {detailRow && (
        ...
        >
          <div
            className="w-full max-w-3xl bg-white rounded-2xl border border-slate-100 shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-[10px] font-mono text-slate-400 mt-0.5">
                  {detailRow.wellName} · {detailRow.companyName} ·{" "}
            ...
            <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
              {detailRow.description && (
                <div className="rounded-xl border border-cyan-100 bg-cyan-50/50 p-4">
                  <p className="text-[9px] font-mono text-cyan-700 uppercase tracking-wider mb-1">
                    Deskripsi Laporan
                  </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  {
                    label: "Surveyor",
                    value: detailRow.surveyorName,
                    extra: null,
                  },
                  {
                    label: "Perusahaan",
                    value: detailRow.companyName,
                    extra: null,
                  },
                  {
                    label: "Foto",
                    value: detailRow.photos.length + " lampiran",
                    extra: null,
                  },
                  {
                    label: "Kualitas Air",
                    value: detailRow.waterQuality
                      ? (QUALITY_LABEL[detailRow.waterQuality] ??
                        detailRow.waterQuality)
                      : "-",
                    extra: detailRow.waterQuality,
                  },
                  ...
                  >
                    ...
                    {extra ? (
                      <span
                        className={cn(
                          "mt-1 inline-flex text-[10px] font-mono px-2 py-0.5 rounded-full border",
                          QUALITY_COLOR[extra],
                        )}
                      >
                        {value}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="rounded-xl border border-slate-100 px-3 py-3 bg-white">
                  ...
                  <p className="text-[14px] font-semibold font-mono text-slate-800">
                    {detailRow.waterDepth.toFixed(1)} m
                  </p>
                </div>
                <div className="rounded-xl border border-slate-100 px-3 py-3 bg-white">
                  <p className="text-[9px] font-mono text-slate-400 uppercase mb-1">
                  ...
                  <p className="text-[14px] font-semibold font-mono text-slate-800">
                    {detailRow.waterUsage != null
                      ? detailRow.waterUsage.toFixed(1) + " m3"
                      : "-"}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-100 px-3 py-3 bg-white">
              ...
              {detailRow.status === "PENDING" ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={approveReport.isPending}
                    className="px-3 py-2 bg-emerald-600 text-white text-[12px] font-semibold rounded-xl hover:bg-emerald-700 inline-flex items-center gap-1.5 disabled:opacity-50"
                  >
                ...
                <div className="text-[10px] font-mono text-slate-400 inline-flex items-center gap-1.5">
                  ...
                  {detailRow.approvedAt
                    ? formatDate(detailRow.approvedAt)
                    : "-"}
                </div>
              )}
            </div>
