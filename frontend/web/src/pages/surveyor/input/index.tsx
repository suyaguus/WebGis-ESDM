...
import {
  ClipboardEdit, Radio, CheckCircle2, Camera, Upload,
  AlertTriangle, Droplets, Gauge, Waves, ChevronDown,
} from 'lucide-react';
...
export default function InputPengukuranPage() {
  ...
  const selectedSensor = SURVEYOR_SENSORS.find(s => s.id === selectedSensorId);
  ...
  const handleReset = () => {
    setSelectedSensorId('');
    setWaterLevel(''); setDebit('');
    setPH(''); setTDS(''); setKekeruhan('');
    setKualitasAir(''); setKondisi('baik');
    setCatatan(''); setFotoCount(0);
    setSubmitState('idle');
  };

  const sensorHistory = SURVEYOR_MEASUREMENTS.filter(
    m => m.sensorId === selectedSensorId,
  ).slice(0, 3);
  ...
  return (
    <div className="p-3 md:p-5 space-y-3 md:space-y-4 w-full">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[18px] font-semibold text-slate-800">Input Pengukuran</h1>
          <p className="text-[11px] text-slate-400 font-mono mt-0.5">
            Input data lapangan harian · {new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>
        {submitState === 'success' && (
          <div className="flex items-center gap-2 text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg">
            ...
            <span className="text-[11px] font-mono font-semibold">Data terkirim!</span>
          </div>
        )}
      </div>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-emerald-600 font-mono mt-0.5">
              Sensor {selectedSensor?.code} · {fotoCount} foto · Menunggu verifikasi Admin
            </p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        ...
        <div className="space-y-4">
              ...
              {SURVEYOR_SENSORS.map(sensor => (
                <button
                  ...
                  className={cn(
                    ...
                    sensor.status === 'alert' && selectedSensorId !== sensor.id && 'border-red-200',
                  )}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[13px] font-bold font-mono text-slate-800">{sensor.code}</span>
                    {sensor.status === 'alert' && <AlertTriangle size={12} className="text-red-500" />}
                    {selectedSensorId === sensor.id && <CheckCircle2 size={12} className="text-blue-600" />}
                  </div>
                  <p className="text-[10px] text-slate-500 mb-1">{sensor.location}</p>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className={cn(
                      'text-[9px] font-mono px-1.5 py-0.5 rounded border',
                      sensor.type === 'gnss' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-blue-50 text-blue-700 border-blue-200',
                    )}>
                  ...
                  <p className={cn('text-[10px] font-mono font-semibold mt-1.5', getSubsidenceColor(sensor.subsidence))}>
            ...
            <div className="p-4 space-y-4">
              ...
              {!isGNSS && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[9px] font-mono text-slate-400 uppercase tracking-wider block mb-1.5 flex items-center gap-1">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={cn(
                          ...
                          selectedSensorId ? 'border-slate-200 focus:border-blue-400' : 'border-slate-100 cursor-not-allowed opacity-60',
                        )}
                      />
              ...
              {isGNSS && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold text-amber-800">Sensor GNSS — tanpa data air tanah</p>
                    <p className="text-[10px] text-amber-600 font-mono mt-0.5">
                      Untuk sensor GNSS, catat kondisi fisik alat dan upload foto dokumentasi.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <div className="border-t border-slate-50 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        />
                        <p className="text-[9px] text-slate-400 font-mono mt-0.5">Batas: &lt; 500 mg/L</p>
                      </div>
                      <div>
                        <label className="text-[9px] font-mono text-slate-400 block mb-1.5">
                          Kekeruhan (NTU)
                        ...
                        />
                        <p className="text-[9px] text-slate-400 font-mono mt-0.5">Batas: &lt; 5 NTU</p>
                      </div>
                    </div>
                  </div>
          ...
          <Card padding={false}>
            ...
            <div className="p-4">
              <p className="text-[10px] text-slate-400 font-mono mb-3">
                Minimal 1 foto · Wajib: foto sensor, kondisi fisik, dan sekitar lokasi
              </p>
              <div className="grid grid-cols-4 gap-2 mb-3">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div
                    ...
                    onClick={() => i < fotoCount ? handleFotoRemove(i) : undefined}
                  >
                    {i < fotoCount ? (
                      <div className="flex flex-col items-center gap-1">
                        <Camera size={14} className="text-blue-500 group-hover:text-red-400 transition-colors" />
              ...
              {fotoCount === 0 && (
                <p className="text-[9px] font-mono text-red-500 mt-1.5 text-center">* Minimal 1 foto wajib diunggah</p>
              )}
            </div>
          </Card>
          ...
          >
            {submitState === 'idle'       && <><ClipboardEdit size={15} /> Submit Pengukuran</>}
            {submitState === 'submitting' && <><span className="animate-spin">⏳</span> Mengirim...</>}
            {submitState === 'success'    && <><CheckCircle2 size={15} /> Berhasil Dikirim!</>}
          </button>

          {!isFormValid && submitState === 'idle' && (
            <p className="text-[10px] text-slate-400 font-mono text-center -mt-2">
              Lengkapi: sensor {selectedSensorId ? '✓' : '✗'} · data pengukuran {(isGNSS || waterLevel) ? '✓' : '✗'} · foto {fotoCount >= 1 ? '✓' : '✗'}
            </p>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <Card padding={false} className="flex flex-col">
            <SectionHeader title="Ringkasan Input" icon={<ClipboardEdit size={13} />} accent="#3B82F6" />
            <div className="p-4 space-y-3">
              {[
                { label: 'Sensor',        value: selectedSensor?.code ?? '—' },
                { label: 'Lokasi',        value: selectedSensor?.location ?? '—' },
                { label: 'Tipe',          value: selectedSensor ? (selectedSensor.type === 'gnss' ? 'GNSS' : 'Air Tanah') : '—' },
                { label: 'Muka Air',      value: waterLevel ? `${waterLevel} m` : '—', hidden: isGNSS },
                { label: 'Debit',         value: debit ? `${debit} m³/hr` : '—', hidden: isGNSS },
                { label: 'pH',            value: pH ? pH : '—', hidden: isGNSS },
                { label: 'TDS',           value: tds ? `${tds} mg/L` : '—', hidden: isGNSS },
                { label: 'Kekeruhan',     value: kekeruhan ? `${kekeruhan} NTU` : '—', hidden: isGNSS },
                { label: 'Kondisi Fisik', value: KONDISI_OPTIONS.find(o => o.value === kondisi)?.label ?? '—' },
              ...
              <div className="border-t border-slate-100 pt-3 space-y-1.5">
                {[
                  ...
                  { label: 'Data pengukuran',  ok: isGNSS || (waterLevel !== '' && debit !== '') },
                  { label: 'Minimal 1 foto',   ok: fotoCount >= 1 },
                ].map(({ label, ok }) => (
                  <div key={label} className="flex items-center gap-2">
                    <span className={cn('text-[10px]', ok ? 'text-emerald-500' : 'text-slate-300')}>{ok ? '✓' : '○'}</span>
                    <span className={cn('text-[10px] font-mono', ok ? 'text-emerald-700' : 'text-slate-400')}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {selectedSensorId && (
            <Card padding={false} className="flex flex-col">
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-2.5 border-b border-slate-100 hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex items-center gap-2">
                  <span className="text-[13px] font-semibold text-slate-800">Riwayat Sensor Ini</span>
                  <span className="text-[9px] font-mono text-slate-400">{sensorHistory.length} TERAKHIR</span>
                </div>
                <ChevronDown size={14} className={cn('text-slate-400 transition-transform', showHistory && 'rotate-180')} />
              </button>

              {showHistory && (
                <div className="divide-y divide-slate-50">
                  {sensorHistory.length === 0 ? (
                    <p className="px-4 py-4 text-[11px] text-slate-400 font-mono text-center">Belum ada riwayat</p>
                  ) : sensorHistory.map(m => (
                    <div key={m.id} className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-mono text-slate-500">{m.date} · {m.submittedAt}</span>
                        <span className={cn(
                          'text-[9px] font-mono px-1.5 py-0.5 rounded border',
                          ...
                          {m.status === 'verified' ? 'VERIFIED' : m.status === 'rejected' ? 'REJECTED' : 'PENDING'}
                        </span>
                      </div>
                      {m.waterLevel !== 0 && (
                        <p className="text-[11px] font-mono text-blue-600 font-semibold">MAT: {m.waterLevel} m · {m.debit} m³/hr</p>
                      )}
                      <p className="text-[10px] text-slate-400 mt-0.5 line-clamp-1">{m.catatan}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
            <p className="text-[11px] font-semibold text-blue-800 mb-2">💡 Tips Pengukuran</p>
            <ul className="space-y-1.5">
              {[
                ...
                <li key={i} className="text-[10px] text-blue-700 font-mono flex items-start gap-1.5">
