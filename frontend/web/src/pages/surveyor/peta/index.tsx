        ...
        >
          <span className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-slate-400">
              {SURVEYOR_SENSORS.length} sensor
            </span>
          </span>
          {filterOpen ? (
        ...
        {filterOpen && (
          <div className="px-4 pb-3 border-t border-slate-50 divide-y divide-slate-50">
            {SURVEYOR_SENSORS.map((s) => (
              ...
              >
                <span className="text-[12px] font-bold font-mono text-slate-800 w-16 flex-shrink-0">
                  {s.code}
                </span>
            ...
            <div className="px-4 py-3 border-b border-slate-100 flex-shrink-0">
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  {
                    label: "Online",
                    count: SURVEYOR_SENSORS.filter((s) => s.status === "online")
                      .length,
                    color: "text-emerald-600 bg-emerald-50 border-emerald-200",
                  },
                  {
                    label: "Alert",
                    count: SURVEYOR_SENSORS.filter((s) => s.status === "alert")
                      .length,
                    color: "text-red-600 bg-red-50 border-red-200",
                  },
                  {
                    label: "AT",
                    count: SURVEYOR_SENSORS.filter((s) => s.type === "water")
                      .length,
                    color: "text-blue-600 bg-blue-50 border-blue-200",
                  },
                  {
                    label: "GNSS",
                    count: SURVEYOR_SENSORS.filter((s) => s.type === "gnss")
                      .length,
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono text-slate-600">
                      Muka Air:{" "}
                      {selected.staticWaterLevel
                        ? `${selected.staticWaterLevel}m`
                        : "-"}
                    </span>
                    <span
                      className={cn(
          ...
          {selected && (
            <div className="absolute bottom-2 left-2 right-2 z-[1000] bg-white border border-blue-200 rounded-xl shadow-lg px-3 py-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => setSelected(null)}
                  className="text-slate-400 w-6 h-6 flex items-center justify-center"
                >
                  <X size={14} />
                </button>
              </div>
