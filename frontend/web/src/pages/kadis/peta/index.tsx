...
export default function KadisPetaPage() {
  ...
  }, [sensors, search, statusF, wellTypeF, companyF]);

  const company = (id: string) => companies.find((c) => c.id === id);

  return (
    <div
      className="flex flex-col md:flex-row h-full min-w-0 overflow-hidden"
      style={{ minHeight: 0 }}
    >
      {/* ── Sidebar ── */}
      {sidebarOpen && (
          ...
          <div className="px-4 py-3 border-b border-slate-100 flex-shrink-0">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[13px] font-semibold text-slate-800 flex items-center gap-1.5">
                <Layers size={13} className="text-emerald-600" /> Filter Sensor
              </span>
              <span className="text-[10px] font-mono text-slate-400">
                {filtered.length} sumur
              </span>
            </div>
          ...
          <div className="absolute bottom-2 left-2 right-2 sm:bottom-4 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-[1000] bg-white border border-emerald-200 rounded-xl shadow-lg px-3 py-3 sm:px-5 sm:py-4 w-auto sm:min-w-[320px] max-w-[calc(100vw-1rem)]">
            ...
            <div className="grid grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-1.5">
              {[
                ...
                ["Perusahaan", company(selected.companyId)?.name ?? "-"],
                ["Unit Bisnis", selected.businessName ?? "-"],
                [
                  "Tren Muka Air",
                ...
                [
                  ...
                  selected.staticWaterLevel != null
                    ? `${selected.staticWaterLevel.toFixed(2)} m`
                    : "-",
                ],
