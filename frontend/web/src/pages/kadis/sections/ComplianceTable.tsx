  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
      <SectionHeader
        ...
        accent="#059669"
        subtitle={`${MOCK_COMPANIES.length} TERDAFTAR`}
        action={
          <button onClick={() => setActivePage('kadis-perusahaan')}
      ...
      <div className="overflow-x-auto flex-1">
        <table className="w-full" style={{ minWidth: 480 }}>
          <thead className="bg-slate-50/60 border-b border-slate-100">
            <tr>
              {['Perusahaan', 'Wilayah', 'Subsidence', 'Kuota', 'Sensor', 'Status'].map(h => (
                <th key={h} className="text-[9px] font-mono text-slate-400 uppercase tracking-wider px-4 py-2.5 text-left whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-50">
            {MOCK_COMPANIES.map(c => {
              ...
              return (
                <tr key={c.id} className={cn('hover:bg-slate-50/40 transition-colors', isOver && 'bg-red-50/30')}>
                  ...
                  <td className="px-4 py-2.5 text-[11px] text-slate-500 font-mono">{c.region}</td>
                  <td className="px-4 py-2.5">
                    <span className={cn('text-[11px] font-semibold font-mono', getSubsidenceColor(c.avgSubsidence))}>
                      {c.avgSubsidence.toFixed(2)}
                    </span>
                    <span className="text-[9px] text-slate-400 font-mono ml-1">cm/thn</span>
                  </td>
                  <td className="px-4 py-2.5">
